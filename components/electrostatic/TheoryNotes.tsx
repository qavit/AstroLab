  ...
  return (
    <>
      ...
      <div className="theory-body" data-testid="electrostatic-theory">
        ...
        <p>
          <strong>電場線</strong>沿著各位置的電場方向延伸，線上的箭頭就是 <Inline>{"\\vec E"}</Inline> 的方向：一般從正電荷出發，指向負電荷或觀察範圍邊界。
          在某些對稱的平面切面中，電場線也可能趨近一個電場為零的點；在零場點 <Inline>{"\\vec E = \\vec 0"}</Inline>，電場線的方向沒有定義，畫面上的線就停在那裡。
          它不是帶電粒子的運動軌跡；粒子的實際運動還取決於電荷正負、質量與初速度。畫面上的線條數量是視覺化的選擇，不是精確的場強量測。
        </p>
        <p>
          例如四顆相同正電荷放在正方形四角時，中心是完整三維庫侖場的<strong>鞍點</strong>：在 <Inline>{"z = 0"}</Inline> 這個對稱平面內，中心附近的電場方向都指向中心；
          若稍微離開平面，電場則沿 <Inline>{"\\pm z"}</Inline> 指離中心。零場點本身沒有方向，所以不能說某一條電場線會唯一地「穿過」中心繼續延伸。
        </p>

        <h2>5. 怎麼讀畫面上的箭頭？</h2>
        <p>
          背景的小箭頭使用非線性顯示尺度，方便同時看見強場與弱場；測量點旁的彩色貢獻向量則使用同一線性比例，因此可以直接看它們怎麼相加。
          箭頭方向代表電場方向；背景箭頭的視覺強弱只用來方便比較，不是線性的場強刻度。真正的 N/C 數值請看測量讀值。
          測量讀值是實際計算的結果，沒有被截斷。5000 N/C 只是背景箭頭顯示尺度的上端；實際計算值不會被截成 5000 N/C。
        </p>
        <FieldLegend />
        <details className="theory-advanced">
